...
int* p2 = p1;
```
- 포인터 변수 p1이 val 변수의 메모리 주소를 가리킵니다. 
- 포인터 변수 p2에 p1의 값을 대입합니다. 
- 결과적으로 p1과 p2 두 포인터 모두 동일한 변수 val의 주소를 가리키게 됩니다.

```
(*p1) += 5;
```
- p1이 가리키는 곳(val)의 값을 5만큼 증가시킵니다.
- val은 20에서 25가 됩니다.

```
(*p2) *= 2;
```
- p2가 가리키는 곳(역시 val)의 값에 2를 곱합니다.
- val은 25에서 50이 됩니다.
